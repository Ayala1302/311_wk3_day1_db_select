...
Answer: SELECT \* FROM users WHERE city = 'Chicago'

2. Select the "state" field for users with a first name of "Glory"

Answer: SELECT state FROM users WHERE first_name LIKE 'Glory'

3. Select all fields from users whose phone1 starts with "773"

Answer: SELECT \* FROM users WHERE phone1 LIKE '773%'
...
2.  - How many rows were returned: 2
    - What states do they live in: Texas(TX) & Maryland(MD)
